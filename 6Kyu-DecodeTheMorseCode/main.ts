...
export default function decodeMorse(morseCode) {
  ...
  from(morseCode).pipe(
    ...
    map((code: string[]): string => code.join('')),

    map((code: string): string => MORSE_CODE[code]),

    reduce(({sentence}: {sentence: string}, letter: string): {sentence: string} => {
    ...
    (answer: string): void => {solution = answer}
  )
    ...
    return Observable.create((observer: Observer<any>) => {
      const subscription = source.subscribe(
        (value: any) => {
          try {
            observer.next(value)
            if (predicate(value)) { subject.next(true); }
          } catch (err) {
            observer.error(err)
          }
        },
